document.addEventListener('DOMContentLoaded', () => {
    ...
    function openReturnPopup(button) {
        ...
        if (!popup) {
            console.error("Return popup element not found"); // 팝업 요소 없음
            return;
        }
    ...
    function confirmReturn() {
        if (!bookId) {
            console.error("No book ID to return");
            return;
        }
    ...
    document.querySelectorAll('.button[data-book-id]').forEach(button => {
        button.addEventListener('click', () => openReturnPopup(button));
    });

    document.querySelector('.popup.return-popup button[onclick="confirmReturn()"]').addEventListener('click', confirmReturn);
});
